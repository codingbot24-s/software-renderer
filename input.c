...
// this is simple function for handling keyboard input and moving models and
// scaling and rotating it
void handle_input(vec3 *translation, vec3 *rotation, float *scale,
                  float delta_time) {
  const bool *key_state = SDL_GetKeyboardState(NULL);
  const float linear_step = (key_state[SDL_SCANCODE_LSHIFT] != 0)
                                ? 0.25f * delta_time
                                : 1.0f * delta_time;
  const float angular_step = (key_state[SDL_SCANCODE_LSHIFT] != 0)
                                 ? 24.0f * delta_time
                                 : 48.0f * delta_time;
  ...
  if (key_state[SDL_SCANCODE_KP_PLUS])
    *scale += linear_step;
  if (key_state[SDL_SCANCODE_KP_MINUS])
    *scale -= linear_step;
}
